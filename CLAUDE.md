...
Env-only, fails closed (no hardcoded fallbacks). `USERS` in server.js: `conor@astoriaadvertising.co` (PW_CONOR) and `client@sicoraconsulting.com` (PW_SICORA — **placeholder email, replace with the real Sicora login**).

## Env vars (Railway)

`WINDSOR_API_KEY`, `ANTHROPIC_API_KEY`, `PW_CONOR`, `PW_SICORA`, optional `MODEL` (default claude-sonnet-4-6). See `.env.example`.
